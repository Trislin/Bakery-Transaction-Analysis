...
cluster_wkday_cbind$cluster_no<-as.factor(cluster_wkday_cbind$cluster_no)
cluster_wkday_cbind %>%
  ggplot( aes(x=temp, fill=cluster_no)) + facet_wrap(~ cluster_no) +
...
cluster_wkday_cbind$cluster_no<-as.factor(cluster_wkday_cbind$cluster_no)
cluster_wkday_cbind %>%
  ggplot( aes(x=rhum, fill=cluster_no)) + facet_wrap(~ cluster_no) +
    geom_density( color="#e9ecef", alpha=0.6, position = 'identity') +
    scale_fill_brewer(palette = 'BrBG') +
    theme_minimal() +
    labs(fill="")
...
cluster_wkday_cbind$cluster_no<-as.factor(cluster_wkday_cbind$cluster_no)
cluster_wkday_cbind %>%
  ggplot( aes(x=wspd, fill=cluster_no)) + facet_wrap(~ cluster_no) +
    geom_density( color="#e9ecef", alpha=0.6, position = 'identity') +
    scale_fill_brewer(palette = 'BrBG') +
    theme_minimal() +
    labs(fill="")
...
```
cluster 1 represents in the non-rush-hours in non-holidays, with a mean temperature of around 7, relatively low wind speed , high humidity, and low number of transactions in different item types. 
...
```
Cluster 2 represents the rush hours in non-holidays, with relatively high temperature, relatively high wind speed, relatively low humidity, and relatively high number of transactions of different items. 
...
grid.arrange(p1,p2,p3,p4,p5,p6, nrow=3,ncol=2)
```
Cluster 3 represents holiday data which has even lower number of transactions comparing to cluster 1 (non-rush hour in non-holiday) even though temperature is relatively higher, humidity is relatively lower.
...
To compare cluster 2 and 4 which are both rush hours in non-holidays, cluster 4 includes lower temperature, lower wind speed, lower humidity, and relatively lower number transactions of items except for bread. 


In conclusion: 

During non-holidays,in rush hours in weekdays with lower temperature, there tends to be relatively fewer number of transactions in items except for bread comparing to those days with higher temperature, even if the wind speed is lower or humidity is lower.  

Comparing to cluster 1, which represents non-rush hours, overall number of transactions is less than that in rush hours as represented by both cluster 1 and 3.

Comparing to cluster 4, which represents holiday, overall number of transactions is the least among all clusters. 
...
grid.arrange(p1,p2,p3,p4,p5,p6, nrow=4,ncol=2)
```
cluster 1 in the weekend data represents (mostly) rush hours in non-holidays with relatively low temperature and low wind speed. The number of transaction in cluster 1 is lower than that in cluster 2. 
...
grid.arrange(p1,p2,p3,p4,p5,p6, nrow=3,ncol=2)
```
cluster 2 in the weekend data represents (mostly) rush hours in non-holidays with relatively high temperature, high wind speed and high humidity. The number of transaction in cluster 2 is highest among all transactions. 

Comparing cluster 2 and cluster 1 which both contain data in (mostly) rush hours in non-holidays, cluster 2 has higher number of transactions probably due to higher temperature, even though the wind speed is higher. 
...
grid.arrange(p1,p2,p3,p4,p5,p6, nrow=3,ncol=2)
```
Cluster 3 in weekends data represents holiday data. The number of transaction in this cluster is the least. 

In conclusions, in non-holidays, temperature seems to be more important than wind speed to number of transactions. Holiday season tends to have lower sales since the bakery is closed on holidays. We need more data to get insights to holiday information. 
...
```{r}
library(amap)
d <- Dist(cluster_wkday_scale, method = "correlation")
hc.complete<- hclust(d, method="complete")
plot(hc.complete, cex = 1, hang = -1)
rect.hclust(hc.complete, k = 5, border = 2:6)

```

```{r}
d <- Dist(cluster_wkend_scale, method = "correlation")
hc.complete <- hclust(d, method="complete")
plot(hc.complete, cex = 1, hang = -1)
rect.hclust(hc.complete, k = 5, border = 2:6)
...
```{r}
fviz_nbclust(cluster_wkday_scale, hcut, method = "wss") +
  labs(subtitle = "Elbow method")
fviz_nbclust(cluster_wkday_scale, hcut, method = "silhouette") +
...
```{r}
fviz_nbclust(cluster_wkend_scale, hcut, method = "wss") +
  labs(subtitle = "Elbow method")
fviz_nbclust(cluster_wkend_scale, hcut, method = "silhouette") +
  labs(subtitle = "Silhouette method")
...
In general, one of the clusters focuses on the transactions out of the rush hours. The other two clusters are split based on weather conditions. One thing worth-noting is that the center of one cluster has humidity equals to 91. In other words, that clusters most likely contains the rush hour transactions during raining days.

It is difficult to draw recommendations directly from the segment of transactions. However, it provides the insight about the similarity of different transactions.

# 5 Conclusion & Next Steps

In a nutshell, with the bakery transaction data and weather record data, we first explored how different features influencing the number of transactions, especially those weather-related variables. Then we dived into the association rules from the transaction history, identified purchasing patterns, and provided recommendations for the bakery to further increase sales. Lastly we tried to cluster the transactions into different segments, and discussed the initial results.
...
3. We could dive deeper into the clustering analysis, to further explore the similarities of transactions within the same cluster.
