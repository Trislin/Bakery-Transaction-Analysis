...
df_merged_cluster[,c(2:11)]<-sapply(df_merged_cluster[,c(2:11)], as.numeric)
...
As cake is the center of the associations, we are also curious when the sales of cakes peak and how the sales of different drinks associated with cakes vary by the hours of the day. 

```{r}
p3<-mba_wkday%>%filter(Item=='Cake')%>%
  group_by(as.factor(Hour))%>%
...
p4<-mba_wkday%>%filter(Item=='Coffee')%>%
  group_by(as.factor(Hour))%>%
  mutate(Avg_No_of_transaction = length(unique(Transaction))/length(unique(mba_wkend$Transaction))) %>%
  ggplot(aes(x = as.factor(Hour), y = Avg_No_of_transaction, fill = as.factor(Hour))) +
  geom_bar(stat = 'identity',alpha=0.8) +
...
p6<-mba_wkday%>%filter(Item=='Hot chocolate')%>%
  group_by(as.factor(Hour))%>%
  mutate(Avg_No_of_transaction = length(unique(Transaction))/length(unique(mba_wkend$Transaction))) %>%
  ggplot(aes(x = as.factor(Hour), y = Avg_No_of_transaction, fill = as.factor(Hour))) +
  geom_bar(stat = 'identity',alpha=0.8) +
...
```
From the figures above, coffee is sold more in the morning, as opposed to tea and hot chocolate. Sales of cake peak in the afternoon, and so as tea and hot chocolate. It is likely that in weekends, people hang out together at the bakery to get the cake and multiple drinks. The bakey could make a special weekend afternoon combo for cake and drinks.
...
p2<-mba_wkday%>%filter(Item=='Farm House')%>%
  group_by(as.factor(Hour))%>%
  mutate(Avg_No_of_transaction = length(unique(Transaction))/length(unique(mba_wkend$Transaction))) %>%
  ggplot(aes(x = as.factor(Hour), y = Avg_No_of_transaction, fill = as.factor(Hour))) +
  geom_bar(stat = 'identity',alpha=0.8) +
