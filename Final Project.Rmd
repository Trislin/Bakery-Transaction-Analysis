...
df_merged_clas <- subset(df_merged_clas, select = -c(Date))
...
df_merged_mba_item_type_temp <- subset(df_merged_mba,
                                       select = c(Transaction, Item_Type_Name))
...
df_merged_mba_item <- df_merged_mba %>%
  group_by(Transaction) %>%
  summarise(basket = as.vector(list(Item)))
...
train <- sample(1:nrow(df_merged_reg), round(0.8*dim(df_merged_reg)[1]))
num_features <- dim(df_merged_reg)[2]
...
for (i in seq(1, nrow(search_grid))){
  rf_reg <- randomForest(No_of_transaction~.,
                           df_merged_reg[train, ],
                           mtry = search_grid[i, ]$mtry,
                           ntree = search_grid[i, ]$ntree,
                           importance = TRUE
                           )
  if (mean(rf_reg$mse) < min_RMSE) {
    min_RMSE <- mean(rf_reg$mse)
...
rf_reg_best <- randomForest(No_of_transaction~.,
                           df_merged_reg[train, ],
                           mtry = best_mtry,
                           ntree = best_ntree,
                           importance = TRUE
...
for (i in seq(1, nrow(search_grid))){
  ...
  boost.fit <- gbm(No_of_transaction~. ,
                  data = df_merged_reg[train, ], 
  ...
  search_grid[i, ]$min_RMSE <- min(boost.fit$cv.error)
  search_grid[i, ]$optimal_trees = match(min(boost.fit$cv.error), boost.fit$cv.error)
...
boost.best <- gbm(No_of_transaction~. ,
                  data = df_merged_reg[train, ], 
                  distribution = "gaussian",
                  cv.folds = 10,
                  n.trees = 4000,
                  ...
                  n.cores = NULL,
                  verbose = FALSE,
                  shrinkage = boost_best_para$shrinkage,
                  interaction.depth = boost_best_para$interaction.depth,
                  n.minobsinnode = boost_best_para$n.minobsinnode,
                  bag.fraction = boost_best_para$bag.fraction,
                  )
...
Then we compare the performance of both the random forest model and the trees model using boosting algorithm on the validation set.

```{r}
boost_pred_test <- predict(boost.best,
                           newdata = df_merged_reg[-train, ],
                           n.trees = 4000)
boost_RMSE_test <- caret::RMSE(boost_pred_test, df_merged_reg[-train, ]$No_of_transaction)

rf_pred_test <- predict(rf_reg_best,
                        newdata = df_merged_reg[-train, ])

rf_RMSE_test <- caret::RMSE(rf_pred_test, df_merged_reg[-train, ]$No_of_transaction)

print(paste0('RMSE of the boosting model on testing set:', round(boost_RMSE_test, 4)))
print(paste0('RMSE of the random forest model on testing set:', round(rf_RMSE_test, 4)))
...
```{r}
par(mfrow = c(2, 2))
partialPlot(rf_reg_best, df_merged_reg[train, ], x.var = 'wdir')
partialPlot(rf_reg_best, df_merged_reg[train, ], x.var = 'wspd')
partialPlot(rf_reg_best, df_merged_reg[train, ], x.var = 'rhum')
partialPlot(rf_reg_best, df_merged_reg[train, ], x.var = 'temp')

```

From the partial dependence plot, we could learn that:

1. Higher temperature has positive impact on the sales
...
3. Higher humidity has negative impact on the sales
...
```{r}
transactions_Item <- as(df_merged_mba_item$basket, "transactions")
...
# Reorder by Lift and display
rules_Item_dt <- data.table( lhs = labels( lhs(rules_Item) ), 
                        rhs = labels( rhs(rules_Item) ), 
                        quality(rules_Item) )[ order(-lift), ]
head(rules_Item_dt,10)
#rules_Item_dt
...
1. People who buy toast tends to buy coffee at the same time

2. People who buy Spanish brunch tends to buy coffee at the same time

3. People who buy medialuna tends to buy coffee at the same time

4. People who buy pastry tends to buy coffee at the same time

It seems that Spanish brunch is different from the other short-listed items. Toast, medialuna, and pastry sounds more like breakfast. We plot the distribution by time of the day to validate our observations.
...
1. Promote bundles of toast and coffee, medialuna and coffee, and pastry and coffee from Monday to Friday, boosting the sales of both.
